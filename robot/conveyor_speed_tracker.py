...
def estimateConveyorSpeed(active_trajectories: List[ObjectTrajectory], completed_trajectories: List[ObjectTrajectory]) -> Optional[float]:
    all_trajectories = active_trajectories + completed_trajectories
    
    # Filter trajectories with enough observations
    valid_trajectories = [t for t in all_trajectories if len(t.observations) >= MIN_OBSERVATIONS_TO_ESTIMATE_VELOCITY]
