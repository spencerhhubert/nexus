...
conn.commit()
c.execute(
    """CREATE TABLE IF NOT EXISTS categories (category TEXT PRIMARY KEY, name TEXT, pieces TEXT, kinds TEXT,colors TEXT, categories TEXT)"""
)
...
for kind in c.execute("""SELECT * FROM kinds""").fetchall():
    ...
    if category:
        ...
        c.execute(
            """UPDATE categories SET kinds=? WHERE category=?""",
            (json.dumps(kinds), bl_category),
        )
    else:
        c.execute(
            """INSERT INTO categories VALUES (?,?,?,?,?,?)""",
            (bl_category, bl_category, "", json.dumps([id]), "", ""),
        )
    conn.commit()
