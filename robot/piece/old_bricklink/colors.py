...
def updateColorsTable(db_path: str):
    ...
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS colors (id TEXT PRIMARY KEY, name TEXT, code TEXT)")
    c.executemany("INSERT OR IGNORE INTO colors VALUES (?,?,?)", [(str(x["color_id"]), x["color_name"], x["color_code"]) for x in colors])
    conn.commit()
    conn.close()
