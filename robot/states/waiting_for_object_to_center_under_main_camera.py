...
class WaitingForObjectToCenterUnderMainCamera(BaseState):
    def __init__(
        self,
        global_config,
        ...
        self.logger = global_config["logger"].ctx(
            state="WaitingForObjectToCenterUnderMainCamera"
        )
    ...
    def step(self) -> Optional[SortingState]:
        ...
        timeout_duration = (
            self.global_config["waiting_for_object_to_center_timeout_ms"] / 1000.0
        )
        if current_time - self.timeout_start_ts >= timeout_duration:
            self.logger.info(
                f"TIMEOUT: WAITING_FOR_OBJECT_TO_CENTER_UNDER_MAIN_CAMERA timed out after {timeout_duration}s"
            )
    ...
    def cleanup(self) -> None:
        ...
        self.logger.info(
            "CLEANUP: Cleared WAITING_FOR_OBJECT_TO_CENTER_UNDER_MAIN_CAMERA state"
        )
