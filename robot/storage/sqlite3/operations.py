...
def saveObservationToDatabase(global_config: GlobalConfig, observation_id: str, trajectory_id: str, 
                             timestamp_ms: int, center_x: float, center_y: float, bbox_width: float, 
                             bbox_height: float, full_image_path: str, masked_image_path: str, 
...
def loadObservationFromDatabase(global_config: GlobalConfig, observation_id: str) -> Optional[dict]:
    conn = getDatabaseConnection(global_config)
    cursor = conn.cursor()
    ...
    return {
        ...
        'classification_result': json.loads(row[9]),
        'created_at': row[10]
    }


def getObservationsForTrajectory(global_config: GlobalConfig, trajectory_id: str) -> list[dict]:
    conn = getDatabaseConnection(global_config)
    cursor = conn.cursor()
