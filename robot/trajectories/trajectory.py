...
class ObjectTrajectory:
    def __init__(self, global_config: GlobalConfig, trajectory_id: str, initial_observation: Observation):
    ...
    def getConsensusClassification(self) -> str:
        ...
        for observation in self.observations:
            ...
            if 'items' in result and result['items']:
                ...
                classification_counts[item_id] = classification_counts.get(item_id, 0) + 1
...
def createTrajectory(global_config: GlobalConfig, initial_observation: Observation) -> ObjectTrajectory:
...
def calculateSpatialDistance(obs: Observation, predicted_x: float, predicted_y: float) -> float:
...
def calculateClassificationConsistency(obs: Observation, trajectory: ObjectTrajectory) -> float:
...
def findMatchingTrajectory(global_config: GlobalConfig, new_observation: Observation, 
                          active_trajectories: List[ObjectTrajectory]) -> Optional[ObjectTrajectory]:
    max_time_gap_ms = global_config['trajectory_matching_max_time_gap_ms']
    max_position_distance_px = global_config['trajectory_matching_max_position_distance_px']
    ...
    classification_weight = global_config['trajectory_matching_classification_consistency_weight']
    ...
    for trajectory in active_trajectories:
        ...
        predicted_x, predicted_y = trajectory.getPredictedPosition(new_observation['timestamp_ms'])
        spatial_distance = calculateSpatialDistance(new_observation, predicted_x, predicted_y)
        ...
        classification_score = calculateClassificationConsistency(new_observation, trajectory)
        
        combined_score = spatial_weight * spatial_score + classification_weight * classification_score
