...
def main():
    ...
    def open_doors_sequence():
        ...
        for dm_idx, distribution_module in enumerate(
            ...
            logger.info(
                f"Opening DM{dm_idx} conveyor door to {conveyor_door_open_angle}°"
            )
            ...
            for bin_idx in bin_indices:
                logger.info(
                    f"Opening DM{dm_idx} bin {bin_idx} door to {bin_door_open_angle}°"
                )
    ...
    def close_doors_sequence():
        ...
        for dm_idx, distribution_module in distribution_modules:
            ...
            for bin_idx in bin_indices:
                logger.info(
                    f"Closing DM{dm_idx} bin {bin_idx} door to {bin_door_closed_angle}°"
                )
                if bin_idx == 3:
                    continue
            ...
            logger.info(
                f"Closing DM{dm_idx} conveyor door to {conveyor_door_closed_angle}°"
            )
    ...
    try:
        logger.info(
            f"Starting servo demo with {len(irl_system['distribution_modules'])} distribution modules..."
        )
